...
// ScanCallback is placeholder for different interfaces that may be
...
// during a scan. The RuleMatching method corresponds to YARA's
// CALLBACK_MSG_RULE_NOT_MATCHING mssage.
type ScanCallbackNoMatch interface {
	RuleNotMatching(*Rule) (bool, error)
}

// ScanFinished is used to signal that a scan has finished. The
// ScanFinished method corresponds to YARA's
...
func scanCallbackFunc(message C.int, messageData, userData unsafe.Pointer) C.int {
	...
	case C.CALLBACK_MSG_IMPORT_MODULE:
		if c, ok := cb.(ScanCallbackModuleImport); ok {
			...
			// FIXME: When/how should this buffer be free()d?
